...
PAYMENT_CARD_NUMBER = os.getenv("PAYMENT_CARD_NUMBER", "0000 0000 0000 0000") PAYMENT_CARD_HOLDER = os.getenv("PAYMENT_CARD_HOLDER", "IVANOV IVAN") PAYMENT_CLICK_LINK = os.getenv("PAYMENT_CLICK_LINK", "") PAYMENT_PAYME_LINK = os.getenv("PAYMENT_PAYME_LINK", "")
...
if not (5000 <= TON_GRAM_RATE_UZS <= 50000): print( f"⚠️ ВНИМАНИЕ: TON_GRAM_RATE_UZS={TON_GRAM_RATE_UZS} выглядит подозрительно " f"(ожидается диапазон 5000-50000 сум за 1 GRAM). Проверь переменную на Railway — " f"похоже на опечатку (пропущенная цифра), из-за которой цены аренды будут неверными." )
Комиссия за сделку аренды — в граммах TON, и какой % возвращается арендатору
RENT_FEE_GRAM = float(os.getenv("RENT_FEE_GRAM", "0.1")) RENT_FEE_REFUND_PERCENT = int(os.getenv("RENT_FEE_REFUND_PERCENT", "40"))
...
REQUIRED_CHANNEL = os.getenv("REQUIRED_CHANNEL", "") REQUIRED_CHANNEL_LINK = os.getenv("REQUIRED_CHANNEL_LINK", "")
...
---- Персистентное хранилище (Railway Volume) ----Диск контейнера сбрасывается при каждом деплое — сюда монтируется Volume,
чтобы добавленные через /addgift подарки (JSON + картинки) не терялись.
Локально, без Volume, по умолчанию пишет рядом с проектом (data/persist).
PERSIST_DIR = os.getenv("PERSIST_DIR", os.path.join(os.path.dirname(file), "data", "persist"))
